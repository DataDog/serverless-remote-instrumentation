...
metricThresholds.forEach((obj) =>
  Object.assign(obj, {
    toString: () =>
      `${obj.aggregator} of metric ${obj.metricName} is ${obj.comparison} than ${obj.threshold}`,
  }),
);

describe("Metrics Validations", () => {
  it.each(metricThresholds)(
    "%s",
    async ({ metricName, threshold, comparison, aggregator }) => {
